...
REM --- Configuración
set "DIR_A=tests\large_mixed_data\server_a"
set "DIR_B=tests\large_mixed_data\server_b"
...
REM --- Validate Headers
echo [INFO] Validate headers...
if "%SEPARATOR%"=="" (
    %PY_EXE% -m src.batch.validate_headers --dir-a "%DIR_A%" --dir-b "%DIR_B%" --output "%OUTPUT_DIR%" --config "%CONFIG_FILE%"
) else (
    %PY_EXE% -m src.batch.validate_headers --dir-a "%DIR_A%" --dir-b "%DIR_B%" --output "%OUTPUT_DIR%" --config "%CONFIG_FILE%" --separator "%SEPARATOR%"
)
